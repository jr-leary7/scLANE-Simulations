...
source("./R/functions_PseudotimeDE_GLM.R")
...
##### monitoring #####
# targets::tar_watch(level_separation = 1200, seconds = 120, seconds_max = 360, project = "PseudotimeDE_GLM")
# targets::tar_manifest(script = "_targets_PseudotimeDE.R") %>%
# left_join((targets::tar_progress(store = "store_PseudotimeDE")), by = "name") %>%
...
sims_single_subj <- data.frame(sim_file = list.files("store_simulation/objects/", pattern = "sim_single_*")) %>%
                    ...
                    dplyr::mutate(ref_dataset = gsub("sim_single_", "", sim_file),
                                  ...
                                  PseudotimeDE_res_name = paste0("PseudotimeDE_", ref_dataset, "_DEG_", dyn_gene_freq, "_N_", n_cells)) %>%
                    dplyr::ungroup()
...
PseudotimeDE_symbols <- rlang::syms(sims_single_subj$PseudotimeDE_res_name)
...
list(
  ...
  tar_eval(values = list(symbol = PseudotimeDE_symbols,
                         data_symbol = sims_single_subj_symbol),
           tar_target(symbol, run_PseudotimeDE(data_symbol))),
  tar_render(brain_metrics, "./Reports/PseudotimeDE_GLM_Brain_Metrics.Rmd"),
  tar_render(endo_metrics, "./Reports/PseudotimeDE_GLM_Endocrinogenesis_Metrics.Rmd"),
  tar_render(panc_metrics, "./Reports/PseudotimeDE_GLM_Pancreas_Metrics.Rmd")
)
