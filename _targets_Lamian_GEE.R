...
source("./R/functions_Lamian_GEE.R")
...
sims_multi_subj <- data.frame(sim_file = list.files("store_simulation/objects/", pattern = "sim_multi_*")) %>% 
                   ...
                   dplyr::mutate(ref_dataset = gsub("sim_multi_het_", "", sim_file), 
                                 ...
                                 lamian_res_name = paste0("Lamian_GEE_", ref_dataset, "_DEG_", dyn_gene_freq, "_N_", n_cells, "_SUBJ_", n_subjects, "_OVERLAP_", perc_overlap, "_", sample_alloc), 
...
Lamian_GEE_symbols <- rlang::syms(sims_multi_subj$lamian_res_name)
...
list(
  ...
  tar_eval(values = list(symbol = Lamian_GEE_symbols, 
                         data_symbol = sims_multi_subj_symbol), 
           tar_target(symbol, run_Lamian_GEE(data_symbol))), 
  tar_render(brain_metrics, "./Reports/Lamian_GEE_Brain_Metrics.Rmd"), 
  tar_render(endo_metrics, "./Reports/Lamian_GEE_Endocrinogenesis_Metrics.Rmd"), 
  tar_render(panc_metrics, "./Reports/Lamian_GEE_Pancreas_Metrics.Rmd")
)
