...
##### monitoring #####
# targets::tar_watch(level_separation = 1200, seconds = 120, seconds_max = 360, project = "scLANE_GEE")
# targets::tar_manifest(script = "_targets_scLANE_GEE.R") %>% 
# left_join((targets::tar_progress(store = "store_scLANE_GEE")), by = "name") %>% 
...
sims_multi_subj <- data.frame(sim_file = list.files("store_simulation/objects/", pattern = "sim_multi_*")) %>% 
                   ...
                   dplyr::mutate(ref_dataset = gsub("sim_multi_het_", "", sim_file), 
                                 ...
                                 sclane_res_name = paste0("scLANE_GEE_", ref_dataset, "_DEG_", dyn_gene_freq, "_N_", n_cells, "_SUBJ_", n_subjects, "_OVERLAP_", perc_overlap, "_", sample_alloc), 
...
scLANE_GEE_symbols <- rlang::syms(sims_multi_subj$sclane_res_name)
...
list(
  ...
  tar_eval(values = list(symbol = scLANE_GEE_symbols, 
                         data_symbol = sims_multi_subj_symbol), 
           tar_target(symbol, run_scLANE_GEE(data_symbol))), 
  tar_render(brain_metrics, "./Reports/scLANE_GEE_Brain_Metrics.Rmd"), 
  tar_render(endo_metrics, "./Reports/scLANE_GEE_Endocrinogenesis_Metrics.Rmd"), 
  tar_render(panc_metrics, "./Reports/scLANE_GEE_Pancreas_Metrics.Rmd")
)
